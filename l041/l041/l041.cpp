...
{
    std::ofstream file("./quickhull.ppm");
...
void part2()
{
    ...
    for (size_t i = 0; i < N; i++)
    {
        if (points[i].get_x() < points[min_x].get_x())
            min_x = i;
        if (points[i].get_x() > points[max_x].get_x())
            max_x = i;
    }
    vector<vec2d> convex;
    convex.push_back(points[max_x]);
    convex.push_back(points[min_x]);

    line l(points[min_x], points[max_x]);
    vector<vec2d> points_left;
    vector<vec2d> points_right;
    for (size_t i = 0; i < N; i++)
    {
        if (i == min_x || i == max_x)
            continue;
        if (l.left_right(points[i]) < 0)
    ...
    find_hull(points_right, l, &convex);
    find_hull(points_left, line(points[max_x], points[min_x]), &convex);
